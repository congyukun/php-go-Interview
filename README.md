...
- [GO相关](#go相关-1)
  ...
  - [Go语言中的并发安全性是什么？如何确保并发安全性？](#go语言中的并发安全性是什么如何确保并发安全性)
  - [Go语言中的defer关键字有什么作用？](#go语言中的defer关键字有什么作用)
  - [什么是互斥锁（Mutex）？在Go语言中如何使用互斥锁来保护共享资源？](#什么是互斥锁mutex在go语言中如何使用互斥锁来保护共享资源)
  - [原子操作和锁的区别是什么？](#原子操作和锁的区别是什么)
  - [go语言引用类型有那些？](#go语言引用类型有那些)
  - [Go语言中的select语句是什么？请给出一个使用select语句的示例?](#go语言中的select语句是什么请给出一个使用select语句的示例)
  - [slice扩容策略](#slice扩容策略)
  - [Go 的垃圾回收机制](#go-的垃圾回收机制)
  - [channel 的内部实现是怎么样的](#channel-的内部实现是怎么样的)
  - [对已经关闭的 channel 进行读写，会怎么样](#对已经关闭的-channel-进行读写会怎么样)
  - [并发安全性](#并发安全性)
  - [runtime](#runtime)
  - [指针](#指针)
  - [接口 interface](#接口-interface)
  - [map 类型](#map-类型)
  - [map 为什么是不安全的](#map-为什么是不安全的)
  - [如何实现map线程安全](#如何实现map线程安全)
  - [map 的 key 为什么得是可比较类型的](#map-的-key-为什么得是可比较类型的)
  - [遍历时, map 的 key 为什么是无序的](#遍历时-map-的-key-为什么是无序的)
  - [map的有序遍历](#map的有序遍历)
  - [make 和 new 的区别](#make-和-new-的区别)
  - [return 与 defer](#return-与-defer)
- [REDIS 相关](#redis-相关)
  - [Redis数据类型有哪些](#redis数据类型有哪些)
  - [Redis为什么快](#redis为什么快)
  - [Redis过期策略有哪些？](#redis过期策略有哪些)
  - [内存淘汰机制](#内存淘汰机制)
  - [持久化的两种⽅式？](#持久化的两种式)
  - [Redis 集群方案](#redis-集群方案)
  - [Redis Cluster 了解吗？](#redis-cluster-了解吗)
  - [Redis 的雪崩、穿透和击穿，如何应对？](#redis-的雪崩穿透和击穿如何应对)
  - [Redis互斥锁](#redis互斥锁)
  - [redis 如何实现延迟队列？](#redis-如何实现延迟队列)
  - [秒杀系统设计](#秒杀系统设计)
- [mysql](#mysql)
  - [CHAR 和 VARCHAR 的区别是什么？](#char-和-varchar-的区别是什么)
  - [VARCHAR(100)和 VARCHAR(10)的区别是什么？](#varchar100和-varchar10的区别是什么)
  - [MySQL 主要由下面几部分构成?](#mysql-主要由下面几部分构成)
  - [MyISAM 和 InnoDB 有什么区别？](#myisam-和-innodb-有什么区别)
  - [Mysql事务有哪些？](#mysql事务有哪些)
  ...
  - [MYSQL中哪几种情况会锁表](#mysql中哪几种情况会锁表)
  - [MySQL 锁表的主要场景](#mysql-锁表的主要场景)
    - [1. DDL 操作](#1-ddl-操作)
    - [2. 显式锁表](#2-显式锁表)
    - [3. 存储引擎特性导致](#3-存储引擎特性导致)
    - [4. 其他情况](#4-其他情况)
  - [避免锁表建议](#避免锁表建议)
  - [如何分析 SQL 的性能？](#如何分析-sql-的性能)
- [如何分析 SQL 的性能？](#如何分析-sql-的性能-1)
  - [1. 使用 EXPLAIN 命令](#1-使用-explain-命令)
  - [2. 使用 EXPLAIN ANALYZE](#2-使用-explain-analyze)
  - [3. 使用慢查询日志](#3-使用慢查询日志)
  - [4. 使用 SHOW PROFILE](#4-使用-show-profile)
  - [5. 使用性能监控工具](#5-使用性能监控工具)
  - [6. 常见性能问题及解决方案](#6-常见性能问题及解决方案)
  - [7. 优化建议](#7-优化建议)
  - [mysql索引数据结构？](#mysql索引数据结构)
  - [Mysql 索引类型有哪些？](#mysql-索引类型有哪些)
      - [聚簇索引的优缺点](#聚簇索引的优缺点)
      - [非聚簇索引介绍](#非聚簇索引介绍)
      - [覆盖索引](#覆盖索引)
  - [正确使用索引的一些建议？](#正确使用索引的一些建议)
  - [mysql索引失效的集中情况？](#mysql索引失效的集中情况)
  - [mysql使用建议](#mysql使用建议)
  - [mysql日志类别](#mysql日志类别)
  - [mysql 的主从复制](#mysql-的主从复制)
  - [mysql从库同步复制方式:](#mysql从库同步复制方式)
  - [分库分表有哪些？有什么优缺点](#分库分表有哪些有什么优缺点)
  - [分表如何处理id?](#分表如何处理id)
  - [MySQL主从同步延时问题](#mysql主从同步延时问题)
  - [慢查询该如何优化?](#慢查询该如何优化)
- [网络](#网络)
  - [HTTP 和 HTTPS 有什么区别？](#http-和-https-有什么区别)
  - [当在浏览器上输入一个网址，其内部发生了什么？](#当在浏览器上输入一个网址其内部发生了什么)
  - [http状态码有哪些？](#http状态码有哪些)
  - [Tcp为什么3次握手？](#tcp为什么3次握手)
  - [为什么要四次挥手？](#为什么要四次挥手)
  - [常用的网络攻击手段有哪些？](#常用的网络攻击手段有哪些)
  - [数据结构](#数据结构)
- [PHP相关](#php相关)
  - [什么是fastcgi?](#什么是fastcgi)
  - [php-fpm讲一下？](#php-fpm讲一下)
  - [php-fpm如何优化：](#php-fpm如何优化)
    - [常见异常](#常见异常)
  - [php7 和 php5区别](#php7-和-php5区别)
  - [为什么php7 比php5性能提升了2倍呢？](#为什么php7-比php5性能提升了2倍呢)
- [算法相关](#算法相关)
...
- 使用互斥锁 ：当缓存失效时，不是所有请求都去数据库重建缓存，而是使用互斥锁让只有一个请求去重建缓存，其他请求等待或返回旧值
- 设置随机过期时间 ：给缓存的过期时间添加一个随机值，避免大量缓存同时失效
- 使用Redis集群 ：通过Redis集群提高可用性，避免单点故障导致的全盘崩溃
- 热点数据永不过期 ：对于一些访问频率极高且基本不变的数据，可以设置为永不过期
- 服务降级 ：当缓存失效且数据库压力大时，可以启动服务降级策略，返回默认值或错误提示
...
解决方案 ：
- 设置热点数据永不过期 ：对于一些热点数据，可以设置为永不过期，或者在即将过期前，异步更新缓存
- 使用互斥锁 ：当缓存失效时，不是所有请求都去数据库重建缓存，而是使用互斥锁让只有一个请求去重建缓存，其他请求等待
- 提前更新缓存 ：对于热点数据，可以设置一个较长的过期时间，并在后台定时更新缓存
- 资源隔离 ：为热点数据设置专门的缓存资源池，与其他数据隔离
...
### 3. 存储引擎特性导致
- MyISAM 引擎的写操作（不支持行锁）
- InnoDB 的全表扫描（不走索引的 UPDATE/DELETE）
...
SQL 性能分析是数据库优化的重要环节，以下是几种常用的分析方法：

## 1. 使用 EXPLAIN 命令

EXPLAIN 是分析 SQL 执行计划的最基本工具，可以显示 MySQL 如何执行查询：
...
SHOW PROFILES;
SHOW PROFILE FOR QUERY 1; -- 查看具体某条查询的详情
```

## 5. 使用性能监控工具

- MySQL Workbench 的性能仪表盘
...
- **全表扫描**: 添加适当的索引
- **索引未被使用**: 检查索引设计和查询条件
- **JOIN 操作过多**: 优化表结构或查询逻辑
- **临时表和文件排序**: 添加合适的索引避免排序操作
- **锁等待**: 优化事务设计，减少锁冲突
...
- 定期进行表分析和优化：`ANALYZE TABLE` 和 `OPTIMIZE TABLE`

通过以上方法的综合运用，可以全面分析和优化 SQL 性能问题。
## mysql索引数据结构？
